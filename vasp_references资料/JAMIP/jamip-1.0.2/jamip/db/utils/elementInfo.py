...
def getFullIonicRadius(symbol):
    ...
    """
    return mendeleev_element(symbol).ionic_radii
...
def getIonicRadius(symbol,charge,coordination):
            
    ionic_radius = mendeleev_element(symbol).ionic_radii
    coords = []
    for ionic in ionic_radius:
        if ionic.charge == charge:
            coord = ionic.coordination
            if coord == coordination:
                return ionic.ionic_radius
            else:
                coords.append(coord)
    if coords == 0:
        raise ValueError('Not vaild %d states for %s.'%(charge,symbol))
    else:
        raise ValueError('Not vaild %s coord for %s. Try other coords: %s .'%(coordination,symbol,' '.join(coords)))
