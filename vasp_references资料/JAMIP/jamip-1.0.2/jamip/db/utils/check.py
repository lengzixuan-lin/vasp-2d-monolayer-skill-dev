...
def isOverlap(structure, **kwargs):
    ...
    atoms=deepcopy(structure.atoms)
    
    if isinstance(structure, Structure):
        for atom1 in atoms:
            atoms.remove(atom1)
            for atom2 in atoms:
                if is_overlap_of_positions(formated_atom1=atom1.to_formated_atom(), 
        ...
        for atom1 in atoms:
            atoms.remove(atom1)
            for atom2 in atoms:
                if is_overlap_of_positions_for_molecule(formated_atom1=atom1.to_formated_atom(), 
                                           formated_atom2=atom2.to_formated_atom(), 
                                           precision=precision):
                    return True
    else:
...
def is_inside(position, selective_range, **kwargs):
    ...
    srange=np.array([[0,1],[0,1],[0,1]], dtype=float)
    if np.array(selective_range).shape == (3,): # i.e. [0.24980, 0.31235, 2] 
        for i in range(0, 3):
            if selective_range[2] == i:
                srange[i] = selective_range[:2]
            else:
                tolerances[i] = 0
    elif np.array(selective_range).shape == (3,2):
        srange=np.asarray(selective_range)
    else:
        raise ValueError('unknown selective_range')

    result = (position - srange[:,0] >= -tolerances) & (srange[:,1] - position >= tolerances)
    return result.all()
...
def check_symbol(symbol_of_element):
    ...
    """
    from .elementInfo import elements
